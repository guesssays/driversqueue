...
│   │   ├── 0.mp3 ... 9.mp3
│   ├── numbers/
│   │   ├── 1.mp3 ... 999.mp3 (числа без ведущих нулей)
│   ├── ordinals/
│   │   ├── 1.mp3 (birinchi)
│   │   ├── 2.mp3 (ikkinchi)
│   │   ├── 3.mp3 (uchinchi)
...
### Папка digits/ (обязательно, используется как fallback):

- **0.mp3 ... 9.mp3** - озвучка цифр от 0 до 9
- Используется как fallback если numbers/{n}.mp3 отсутствует
- Для uzLat/uzCyr также используется как fallback если ordinals отсутствует

### Папка numbers/ (рекомендуется, для естественной озвучки номеров):

- **1.mp3 ... 999.mp3** - озвучка чисел от 1 до 999 без ведущих нулей
- Используется для номеров талонов: "T001" озвучивается как "T 1", "T228" как "T 228"
- Если файл отсутствует для конкретного числа → fallback на digits/ (цифра за цифрой)
- Пример: если numbers/228.mp3 отсутствует, "T228" будет озвучено как "T 2 2 8"
- Диапазон: 1-999 (для чисел 0 используется digits/0.mp3)
...
**Если numbers/1.mp3 доступен (рекомендуется):**
1. `beep.mp3` - сигнал (опционально)
2. `number.mp3` - "Navbat raqami" / "Навбат рақами"
3. `letters/T.mp3` - "T"
4. `numbers/1.mp3` - "1" (число без ведущих нулей)
5. `ordinals/3.mp3` - "uchinchi" / "учинчи" (порядковое числительное для окна 3)
6. `window_suffix.mp3` - "oynaga" / "ойнага"
7. `go.mp3` - опционально (если отсутствует - пропускается)

**Результат:** "Navbat raqami T 1. uchinchi oynaga." / "Навбат рақами T 1. учинчи ойнага."

**Если numbers/1.mp3 отсутствует (fallback):**
1. `beep.mp3` - сигнал (опционально)
2. `number.mp3` - "Navbat raqami" / "Навбат рақами"
3. `letters/T.mp3` - "T"
4. `digits/0.mp3` - "0"
5. `digits/0.mp3` - "0"
6. `digits/1.mp3` - "1"
7. `ordinals/3.mp3` - "uchinchi" / "учинчи"
8. `window_suffix.mp3` - "oynaga" / "ойнага"
9. `go.mp3` - опционально

**Результат:** "Navbat raqami T 0 0 1. uchinchi oynaga." / "Навбат рақами T 0 0 1. учинчи ойнага."

**Fallback логика:**
- Если `numbers/{n}.mp3` отсутствует (404) → используется `digits/` (цифра за цифрой)
...
**Результат:** "Номер T 0 0 1. окно 3."

## Обработка ошибок и fallback

### Для всех языков:

1. **numbers/{n}.mp3 (номера талонов):**
   - Если отсутствует для числа 1-999 → fallback на `digits/` (цифра за цифрой)
   - Пример: если numbers/228.mp3 отсутствует, "T228" озвучится как "T 2 2 8"
   - Если число равно 0, используется digits/0.mp3
...
- **Номера талонов:** ведущие нули удаляются автоматически. "T001" озвучивается как "T 1", "T228" как "T 228". Если файл numbers/{n}.mp3 отсутствует, используется fallback на digits/ (цифра за цифрой)
- **Диапазон чисел:** 1-999. Для чисел вне этого диапазона или невалидных номеров используется fallback на digits/
